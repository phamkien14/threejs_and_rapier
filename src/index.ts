...
import("@dimforge/rapier3d").then((RAPIER) => {
  ...
  // Create Trees
  function addTree (nsubdivs: number, scale: { x: number, y: number, z: number }) {
    const treeMap = new THREE.TextureLoader().load('./texture/tree.png')
    const treeMaterial = new THREE.SpriteMaterial({map : treeMap})
    const tree = new THREE.Sprite(treeMaterial)
    tree.position.set(40,3,20)
      ...
      if (i % 10 == 0)
      {
        for (var j = -50 ; j < 50; j++)
        {
          if (j % 5 == 0)
          {
            ...
            trees[treenum].position.set(i,3,j)
            scene.add(trees[treenum])
            treenum++
          }
        }
      }
